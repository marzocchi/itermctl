...
func AuthDisabled() error {
	...
	if err != nil {
		return fmt.Errorf("itermctl: %w", err)
	}

	info, err := os.Stat(disableAuthFilePath)
	if os.IsNotExist(err) {
		return fmt.Errorf("itermctl: %s does not exist", disableAuthFilePath)
	}

	if !info.Mode().IsRegular() {
		return fmt.Errorf("itermctl: %s exists, but is not a regular file", disableAuthFilePath)
	}

	if info.Sys().(*syscall.Stat_t).Uid != 0 {
		return fmt.Errorf("itermctl: %s exists, but is not owned by root", disableAuthFilePath)
	}
	...
	if scanner.Text() != magicString {
		return fmt.Errorf("itermctl: contents of %s do not match expected", disableAuthFile)
	}
...
func GetCredentials(appName string, activate bool) (*Credentials, error) {
	...
	if activate {
		...
		if !running {
			return nil, fmt.Errorf("get cookie: iTerm2 is not running and activation is disabled")
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("get cookie: %s", err)
	}
...
// Connect connects to iTerm2's socket using the given credentials.
