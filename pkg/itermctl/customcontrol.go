...
type CustomControlSequenceNotification struct {
	Matches []string
	Message *iterm2.CustomEscapeSequenceNotification
}

// CustomControlSequenceMonitor subscribes to Custom Control Sequence Notifications and writes every Notification that
...
func CustomControlSequenceMonitor(ctx context.Context, client *Client, identity string, re *regexp.Regexp, sessionId string) (<-chan CustomControlSequenceNotification, error) {
