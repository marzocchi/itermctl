...
import (
	...
	"mrz.io/itermctl/pkg/itermctl/app"
	"os"
	"text/tabwriter"
)
...
var SendTextCommand = &cli.NestedCommand{
	Command: &cobra.Command{
		...
		RunE: cli.RunWithClient("itermctl", func(conn *itermctl.Client, cmd *cobra.Command, args []string) error {
			...
			err = app.NewApp(conn).SendText(args[0], string(data), suppressBroadcast)
			if err != nil {
				return err
			}
...
var ListSessionsCommand = &cli.NestedCommand{
	Command: &cobra.Command{
		...
		RunE: cli.RunWithClient("itermctl", func(conn *itermctl.Client, cmd *cobra.Command, args []string) error {
			...
			activeSessionId, err := methods.GetActiveSessionId()
			if err != nil {
				return err
			}
...
var SplitPanesCommand = &cli.NestedCommand{
	Command: &cobra.Command{
		...
		RunE: cli.RunWithClient("itermctl", func(conn *itermctl.Client, cmd *cobra.Command, args []string) error {
			...
			sessionIds, err := app.NewApp(conn).SplitPane(args[0], vertical, before)
			if err != nil {
				return err
			}
...
var CreateTabCommand = &cli.NestedCommand{
	Command: &cobra.Command{
		...
		RunE: cli.RunWithClient("itermctl", func(conn *itermctl.Client, cmd *cobra.Command, args []string) error {
			...
			resp, err := app.NewApp(conn).CreateTab(windowId, tabIndex, profileName)
			if err != nil {
				return err
			}
